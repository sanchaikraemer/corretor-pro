...
import { __testarModeloComercialV671 } from "./api/_pipeline.js";
...
const anderson = __testarModeloComercialV671({
  parsed: parsedBase({
...
assert.equal(anderson._schemaComercial,671);

// 2. “Muito obrigado” NÃO apaga retorno futuro confirmado.
const retornoFuturo = __testarModeloComercialV671({
...
const retornoLiteral = __testarModeloComercialV671({
...
const pergunta = __testarModeloComercialV671({
...
const cordial = __testarModeloComercialV671({parsed:parsedBase(),lead:{name:"Ana"},timeline:[msg("Sanchai","Fico à disposição."),msg("Ana","Um abraço")]});
...
const falouPorUltimo = __testarModeloComercialV671({parsed:parsedBase(),lead:{name:"Paulo"},timeline:[msg("Paulo","Pode mandar a proposta"),msg("Sanchai","Enviei a proposta para sua análise.","01/07/2026","10:01")]});
assert.equal(falouPorUltimo.modeloComercial.acao.status,"aguardando-resposta");

// 7. Visita futura é compromisso agendado.
const visita = __testarModeloComercialV671({
...
const vencido = __testarModeloComercialV671({
...
const venda = __testarModeloComercialV671({parsed:parsedBase(),lead:{name:"Isabela"},timeline:[msg("Isabela","Assinei o contrato e mandei o comprovante de pagamento")]});
...
const nova = __testarModeloComercialV671({
...
assert.match(api,/_schemaComercial: 671/);
assert.match(persistence,/oportunidade:\$\{oportunidadeId\}/);

console.log("teste-modelo-comercial-v671: OK");
