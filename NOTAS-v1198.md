# v1198 — o "Período padrão dos áudios" sai da tela do Cérebro (a proteção fica por dentro)
...
Nota de numeração: esta atualização foi preparada como 1196 e depois 1197, mas as duas saíram no
mesmo dia por outra frente de trabalho (biblioteca do login enxuta e desfazer mensagem copiada) —
por isso esta virou 1198.

## Validação

| Verificação | Resultado |
|---|---|
| Suíte completa | 367 testes verdes |
| Teste de comportamento novo | `v1198-periodo-audios-sem-campo-na-tela` (campo ausente da tela; janela de 90 dias viva no servidor e no app; salvar/zerar preserva o valor salvo em vez de cravar 90) |
...
**Documentação:** `NOTAS-v1198.md` (novo)

**Versão:** `package.json`, `package-lock.json`

**Testes:** `tests/v1198-periodo-audios-sem-campo-na-tela.test.mjs` (novo),
