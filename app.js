...
// ===== Atualização #724-3: instrumentação leve de performance =====
...
// ===== Histórico interno do app (Atualização #724-3) =====
...
// Atualização #724-3: a seta física fecha a gaveta antes de sair da tela atual.
...
// Identificador informativo da arquitetura atual de mensagens (diagnóstico/telemetria).
// A checagem de mensagens abaixo NÃO depende mais deste valor bater exatamente —
// isso causava mensagens válidas serem descartadas quando a API gravava uma tag antiga.
const ARQUITETURA_MENSAGENS_ATUAL = "gpt55-v724-3-analise-pura-3-mensagens";
...
function mensagensDaAnalise(a){
  ...
  const alt = (a.mensagens && typeof a.mensagens === "object") ? a.mensagens : {};
  const pick = (key) => {
    const v = m[key];
    if(v == null) return "";
  ...
  // Aceita tanto messages.a/b/c quanto mensagens.recomendada/maisSuave/maisDireta —
  // qualquer um dos dois caminhos que a API/lead tenha preenchido.
  const aMsg = pick("a") || pick("direta") || String(alt.recomendada || "").trim();
  const bMsg = pick("b") || pick("consultiva") || String(alt.maisSuave || "").trim();
  const cMsg = pick("c") || pick("retomada") || String(alt.maisDireta || "").trim();
  ...
  return {
    ...
    aLabel:String(m.aLabel || "").trim() || "Recomendada",
    bLabel:String(m.bLabel || "").trim() || "Mais suave",
    cLabel:String(m.cLabel || "").trim() || "Mais direta",
...
/* ============================================================
   Atualização #724-3 — Tela do lead consolidada
...
function cp704Css(){
  ...
  function cp704Prob(lead){
    ...
    const bruto = ref != null ? Number(ref) : (Number(lead?.probabilityPercent)||Number(lead?.analysis?.probabilityPercent)||0);
    const n = Math.max(0, Math.min(100, Math.round(bruto||0)));
    const etapa = String(lead?.etapa || lead?.analysis?.etapaSugerida || '').toLowerCase();
    const status = String(lead?.analysis?.modeloComercial?.oportunidade?.status || '').toLowerCase();
    const perdido = /perdid|encerr/.test(etapa) || /perdid|encerr/.test(status);
    const temSinalAtivo = !!(
      cp704Text(lead?.analysis?.summary || lead?.summary || lead?.analysis?.nextAction || lead?.nextAction) ||
      (Array.isArray(lead?.recentMessages) && lead.recentMessages.length)
    );
    // v715: nunca mostrar 0% para lead ativo/em descoberta. 0% fica só para perdido/encerrado.
    if(!perdido && n <= 0 && temSinalAtivo) return 35;
    return n;
  }
  ...
  function cp704Msgs(lead){
    const a=lead?.analysis||{};
    let m=(typeof mensagensDaAnalise==='function') ? mensagensDaAnalise(a) : {};
    if(!(m.a && m.b && m.c) && (lead?.messages || lead?.mensagens)){
      // Aceita também lead.messages.a/b/c e lead.mensagens.recomendada/maisSuave/maisDireta
      // gravados direto no lead, fora de lead.analysis.
      const doLead = (typeof mensagensDaAnalise==='function') ? mensagensDaAnalise({ messages: lead.messages, mensagens: lead.mensagens }) : {};
      if(doLead.a && doLead.b && doLead.c) m = doLead;
    }
    const obsFact=cp707ObservationFacts(lead);
...
// Atualização #724-3: card "O que mudou" — antes → agora + por que importa.
...
// Atualização #724-3: o cabeçalho e os indicadores pertencem à tela Hoje, não ao detalhe do lead.
...
// Atualização #724-3: wrapper antigo de renderLeadFoco removido.
...
function cpAppointmentData(lead){
  const aps=lead?.analysis?.confirmedAppointments;
  const a=Array.isArray(aps)&&aps.length ? (aps[0]||{}) : null;
  const raw=a?.quando||a?.dataHora||lead?.analysis?.lembrete?.quando||"";
  let time="Hoje";
  if(raw){
    const dt=new Date(raw);
    if(!Number.isNaN(dt.getTime())) time=dt.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"});
    else { const m=String(raw).match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/); if(m) time=m[0].padStart(5,"0"); }
  }
  const text=[a?.oQue||a?.tipo||"",produtosLabel(lead)||""].filter(Boolean).join(" · ") || cpStage(lead);
  return {time,text};
}
...
function cpNextAction(lead){
  const a=lead?.analysis||{}, d=a?.diagnostico||{};
  return String(a.proximaAcao||a.nextAction||d.proximaAcao||d.ultimaInfoPrometida||motivoCurto(lead)||"Retomar atendimento").replace(/\s+/g," ").slice(0,62);
}
  ...
  if(running){
    running.innerHTML=ordered.length?ordered.slice(0,4).map(l=>{
      ...
      return `<button type="button" class="cp-running-row" onclick='cpOpenLead(${JSON.stringify(id)})'>
        ...
        <span class="cp-chip ${meta.cls}">${cpEscape(cpStage(l))}</span>
...
window.ui670Reanalisar=async function(btn){
  ...
  try{
    ...
    // Sem scrollIntoView aqui: o corretor já está olhando pra esta tela; rolar de volta
    // pro topo dava a impressão de que o app "voltava" sozinho depois de reanalisar.
...
/* ============================================================
   Atualização #724-3 — revisão de auditoria
...
/* ============================================================
   Atualização #724-3 — fechamento real da pendência de performance
...
/* ============================================================
   Atualização #724-3 — acabamento profissional estável
   ============================================================ */
(function(){
...
/* ============================================================
   Atualização #724-3 — Hotfix real mobile
...
/* ============================================================
   Atualização #724-3 — Correção real da lista mobile
...
/* ============================================================
   Atualização #724-3 — Correção definitiva carregamento total Atendimentos
...
/* ============================================================
   Atualização #724-3 — Preparação da Carteira
...
/* ============================================================
   Atualização #724-3 — correção de versão exibida no topo/mobile
...
/* ============================================================
   Atualização #724-3 — estabilidade pós-cache
...
/* ============================================================
   Atualização #724-3 — Preparação da Carteira estável
...
(function(){
  ...
  css.textContent = `
    body.lead-foco-aberto #cp702HomeProgress,body.lead-foco-aberto #cp697HomeProgress{display:none!important}#cp697HomeProgress{display:none!important}.cp702-home-progress{border:1px solid rgba(255,255,255,.10);border-radius:18px;background:rgba(7,52,64,.58);padding:16px;margin:14px 0;display:block!important}.cp702-home-title{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.cp702-home-title b{font-size:16px;color:var(--text);font-weight:950}.cp702-home-title button{border:1px solid rgba(255,107,92,.40);background:rgba(255,107,92,.07);color:var(--lime);border-radius:999px;padding:8px 12px;font-weight:950}.cp702-home-bar{height:10px;border-radius:999px;background:rgba(255,255,255,.08);overflow:hidden}.cp702-home-bar span{display:block;height:100%;background:linear-gradient(90deg,#ff6b5c,#68ff95);border-radius:999px;min-width:3px;transition:width .18s ease}.cp702-home-meta{display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;margin-top:9px;color:var(--muted);font-size:11px;font-weight:850}@media(max-width:760px){.cp702-home-progress{margin:12px 0;padding:15px}.cp702-home-title b{font-size:15px}.cp702-home-title button{font-size:13px;padding:8px 11px}}`;
  document.head.appendChild(css);
...
// Atualização #724-3: bloco antigo da tela orientada à ação removido; renderLeadFoco foi consolidado na função principal.
