...
// escolhidos UMA VEZ (mesma régua de sempre, nada de aleatório) e persistidos — ficam fixos o
// dia inteiro: atender um faz o número CAIR (10→9→8...), sem reposição — só amanhã a dose é
// escolhida de novo. "Atender +1" continua existindo pra quem QUISER puxar mais um da fila de
// propósito, e o extra também fica fixo.
...
if(ehFds){
  ...
  // 1. Primeira leitura do dia escolhe (por ranking, não aleatório) e PERSISTE o top 10 por engajamento.
  ...
  assert.deepEqual(ids2, ids1, 'dose já escolhida não muda com a chegada de um novo candidato melhor');
  ...
  // dose nova é escolhida a partir da fila viva — não fica presa ao valor salvo indefinidamente.
  ...
  assert.ok(!idsAmanha.includes('a') && !idsAmanha.includes('b') && !idsAmanha.includes('c'),
    'quem está marcado como atendido hoje (__hoje) continua fora mesmo numa dose recém-escolhida');
