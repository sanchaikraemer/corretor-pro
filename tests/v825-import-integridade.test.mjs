...
assert.match(app, /Atualizar o cliente existente/);
assert.match(app, /Criar um novo cliente/);
assert.match(app, />Cancelar</);
assert.match(app, /Mesmo nome: nunca funde automaticamente/);
...
assert.match(leadUpdate, /forceNew:\s*true/);
...
assert.ok(atualizarBloco.indexOf('.update(payloadConsolidacao)') < atualizarBloco.indexOf('nova = await analyzeWithBrain'), 'a conversa consolidada deve ser salva antes da reanálise');
