...
//
// v1051 tentou fazer a mensagem também contar (só pra reforçar, nunca afrouxar) depois de um
// caso real onde isso pareceu necessário — mas v1052 — pedido explícito e definitivo do dono:
// "esquece 2 regras, vamos usar uma só, que é de marcar atendimento, esquece a data da última
// msg". Voltou a ser exatamente a regra da v1018: SÓ o atendimento marcado conta, mensagem nunca
// entra nessa conta, nem pra ajudar nem pra atrapalhar.
...
assert.doesNotMatch(janelaSemComentarios, /\bl\.daysSinceLastTouch\b|\bl\.daysSinceClientReply\b|_diasDesdeMsg\(/,
  'o CÓDIGO de emJanelaDeEspera não pode mais olhar nenhum campo baseado em mensagem — só o último atendimento (v1052)');
...
// mensagem recente ou antiga (mensagem não entra mais nessa conta desde a v1018/v1052).
...
// 3. v1018/v1052 — Atendimento manual ANTIGO (2020) NÃO é "ajudado" por uma mensagem recente (2
// dias): mensagem não conta pra nada aqui, nem pra proteger nem pra liberar. Só o atendimento
// manda, e 2020 já passou muito do limiar — não protege.
