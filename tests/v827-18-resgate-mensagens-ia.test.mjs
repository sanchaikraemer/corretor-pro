...
// v1331: entrou um QUARTO ponto de chamada, e ele também não é reprompt de conteúdo — é a etapa
// que ESCREVE as três mensagens no modo de duas etapas (desligado por padrão). Ela recebe o
// diagnóstico pronto, nunca as mensagens já escritas: não existe "corrija o que você escreveu".
// Quem guarda isso é tests/v1331-analise-em-duas-etapas.test.mjs.
assert.equal((bloco.match(/chamarGPT4Json\(/g) || []).length, 4,
  "4 pontos de chamada: principal, repetição por erro passageiro, modelo indisponível e a etapa que escreve as três — nunca um laço de correção");
...
assert.match(bloco, /prompt: promptMensagens/,
  "a quarta chamada é a que escreve as três, com pedido próprio — e nunca recebe mensagem pronta pra consertar");
