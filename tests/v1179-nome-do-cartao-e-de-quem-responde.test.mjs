...
// Sem nome de arquivo utilizável e com mais de um candidato, o app NÃO escolhe ninguém (v1180 —
// antes daqui ele chutava o primeiro autor, que foi exatamente o defeito do print).
assert.equal(guessLeadData(timelinePrint, '', 'export.txt').clientName, 'Cliente não identificado');
// O nome do arquivo nunca inventa: se o contato dele não fala na conversa, ninguém é escolhido.
assert.equal(guessLeadData(timelinePrint, '', 'Conversa do WhatsApp com Familia Praia.zip').clientName, 'Cliente não identificado',
