...
export async function analyzeWithBrain({ lead, timeline, openai, leadId, forcarVariacao = false, modeloMensagens }) {
  if (!openai) {
    return {
      mode: "sem_api",
...
export async function compararEvolucao({ anterior, atual, novasMensagens, openai }) {
  ...
  const trechoNovas = Array.isArray(novasMensagens) && novasMensagens.length
    ? novasMensagens.slice(-40).map(m => `[${m.date||""} ${m.time||""}] ${m.author}: ${m.text}`).join("\n")
    : "(não foi possível isolar as mensagens novas — compare pelo estado geral)";
...
// ETAPA 3 — Analisa: recebe mensagens + transcrições prontas, monta a timeline,
// roda o Cérebro (1 chamada OpenAI) e devolve lead + análise. Sem transcrição aqui.
export async function finalizarAnaliseDaConversa(payload) {
  const { txtFile, messages, audioFilesRelevantes, transcriptionMap, janelaConversa, ignoredFilesCount, ignoredFiles, audiosTotalNoZip, audiosDescartadosPorJanela, metricsBase } = payload;
  const timeline = montarTimelineComTranscricoes(messages || [], audioFilesRelevantes || [], transcriptionMap || {});
  // rawText reconstruído só da janela (o TXT completo de anos não trafega de volta — estoura o limite do POST)
  ...
  return {
    ...
    audioTranscriptions: transcriptionMap || {},
