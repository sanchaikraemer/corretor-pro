...
import { getSupabaseAdmin, persistProcessingResult } from "./_persistence.js";
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    return json(res, 400, { ok: false, error: "GET só suporta action=memoria-get." });
  }
...
async function acaoAtualizarComEvolucao(body, res) {
  ...
  const novasMensagens = timelineNova.filter(m => !chavesAntigas.has(assinaturaMsg(m))).slice(-40);
