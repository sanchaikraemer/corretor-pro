...
export async function listRecentProcessings(limit = 12) {
  const supabase = getSupabaseAdmin();
  ...
  const mapped = (data || []).map(row => {
    ...
    const last = Array.isArray(timeline) ? timeline[timeline.length - 1] : null;
    // Filtra compromissos inventados pela IA também na leitura, usando o texto real
    // da conversa — assim os já salvos somem sem precisar reanalisar tudo.
    if (analysis && Array.isArray(analysis.confirmedAppointments) && analysis.confirmedAppointments.length) {
      const convText = (Array.isArray(timeline) ? timeline.map(m => m && m.text || "").join("\n") : "")
        + "\n" + (row.texto_extraido || "");
    ...
    // "Dias parado" reflete a CONVERSA REAL (WhatsApp/cliente), não anotações manuais do corretor
    // (presencial/ligação/print de histórico). Registrar um histórico antigo NÃO pode zerar o contador
    // como se o cliente tivesse falado hoje. Quem está parado há 50 dias continua aparecendo como parado.
    const ehItemManual = (m) => {
      const s = String(m?.source || ""), t = String(m?.type || "");
      return s === "manual" || s === "crm" || t === "print-whatsapp"
        || ["atendimento", "nota", "ligacao", "visita", "presencial"].includes(t);
    };
    const realMsgs = Array.isArray(timeline) ? timeline.filter(m => !ehItemManual(m)) : [];
    const lastReal = realMsgs.length ? realMsgs[realMsgs.length - 1] : null;
    ...
    // Último TOQUE de QUALQUER tipo (inclui atendimento presencial/ligação do corretor).
    // Serve pra mostrar, ao lado do silêncio da cliente, há quanto tempo VOCÊ agiu — sem
    // mexer no "dias parado" (que continua refletindo só a conversa real do cliente).
    const lastTouchIso = last?.iso || lastIso;
    const daysSinceTouch = diasCalendarioBR(lastTouchIso);
    // Identidade do lead = NOME COMPLETO salvo (+ telefone quando houver). Dois nomes
    // diferentes — mesmo que o primeiro nome seja igual (ex.: "Luciano Bertani" x
    // "Luciano Renaissance") — NUNCA são tratados como o mesmo lead. Antes a chave vinha
    // do nome do arquivo e um cliente novo podia "atualizar" outro só pelo primeiro nome.
    const nomeResolvido = nameFrom(fileName, analysis, row);
    // Última RESPOSTA = a última mensagem da PRÓPRIA cliente (não conta mensagem do corretor/
    // empresa nem anotação manual). Diferente do "último contato" (qualquer toque).
    ...
    const clientMsgs = Array.isArray(timeline) ? timeline.filter(ehClienteMsg) : [];
    const lastClient = clientMsgs.length ? clientMsgs[clientMsgs.length - 1] : null;
    ...
    // Deduplica pelo NOME COMPLETO — mesma pessoa = um card só — valendo tanto pra conversa
    // importada quanto pra lead criado à mão (o corretor reclamava de Bocorni duplicado: ele
    // cadastrava o mesmo cliente mais de uma vez e cada um virava um card). Mantém o registro
    // mais completo (mais mensagens/histórico), não o último criado, pra não esconder o lead já
    // trabalhado atrás de um cadastro novo vazio.
    // Nome genérico/vazio ("Cliente importado") NÃO entra na dedupe (cada um fica único pelo id),
    // senão todos os sem-nome virariam um só.
    ...
    return {
      ...
      // Última atualização do lead = qualquer edição/inclusão (etapa, observação, resposta,
      // nome/telefone, foto, reanálise...). Abrir/fechar o lead NÃO escreve no banco, então não conta.
      ...
      daysSinceClientReply: daysSinceClientReply,
      audiosEncontrados: row.audios_encontrados ?? null,
      audiosTranscritos: row.audios_transcritos ?? null,
      recentMessages: Array.isArray(timeline) ? timeline.map(m => ({ date: m.date, time: m.time, author: m.author, text: m.text, type: m.type, source: m.source, proposta: m.proposta || null, iso: m.iso || null })) : [],
      analyzed,
      analysis
    };
  });
  ...
  const riqueza = (it) => (Array.isArray(it.recentMessages) ? it.recentMessages.length : 0) + (it.analyzed ? 1 : 0);
  ...
  for (const k of ordem) {
    ...
    // Paginação implícita do histórico: a lista/dashboard e a tela do lead só usam
    // as mensagens recentes. Enviar a conversa INTEIRA de cada lead inflava o payload
    // (16 MB+ para centenas de leads, com áudios transcritos) e travava o app no
    // celular ao baixar/parsear. Mantém só as últimas 40 mensagens por lead.
    if (Array.isArray(clean.recentMessages) && clean.recentMessages.length > 40) {
      clean.recentMessages = clean.recentMessages.slice(-40);
    }
    unique.push(clean);
