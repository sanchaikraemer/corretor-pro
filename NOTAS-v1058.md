# NOTAS v1058 — revisão dos prompts do Cérebro usando uma conversa real como caso de teste
...
- `tests/v1058-cerebro-midia-prazo-e-pedido-sem-resposta.test.mjs` (novo): cobre os 5 achados —
...
- `npm test`: suíte inteira verde (`node --check` em todos os arquivos de API + suíte completa,
  incluindo os testes v1056/v1057 que chegaram de outra sessão enquanto esta rodava).
...
sem resposta direta"), `tests/v1058-cerebro-midia-prazo-e-pedido-sem-resposta.test.mjs` (novo),
`package.json`/`package-lock.json` (versão + script `test`), `NOTAS-v1058.md`, versão
**1057 → 1058** (dois números foram usados em paralelo por outra sessão — 1056 e 1057 — enquanto
esta rodava; esta mudança vira 1058 pra não colidir com nenhum dos dois).
