...
test("v036 mantém atualização automática e evita mistura de arquivos em cache", () => {
  assert.match(appSource, /const APP_VERSION = "v036"/);
  ...
  assert.match(workerSource, /corretor-pro-v036/);
  assert.match(htmlSource, /app\.js\?v=036/);
  assert.match(htmlSource, /styles\.css\?v=036/);
  assert.match(appSource, /db\.js\?v=036/);
  assert.match(appSource, /whatsapp\.js\?v=036/);
...
test("versão v036 aparece no cabeçalho superior", () => {
  assert.match(htmlSource, /id="header-version"[^>]*>v036<\/span>/);
...
test("v036 mantém análise objetiva, visual e detalhes recolhidos", () => {
...
test("v036 substitui aviso genérico por confirmação discreta e só mostra alerta acionável", () => {
...
test("v036 mantém sugestões completas, visíveis e numeradas", () => {
...
test("v036 registra Atendido agora sem timer ou bloqueio por tempo", () => {
...
test("v036 mantém análise sempre disponível e remove bloqueios por timer", () => {
...
test("v036 usa o seletor como período da análise, cópia e histórico", () => {
...
test("v036 mostra o horário da movimentação uma única vez no card", () => {
...
test("v036 remove apenas o status de nova mensagem do corretor parceiro no card", () => {
  const renderListStart = appSource.indexOf("function renderList()");
  const renderListEnd = appSource.indexOf("function groupTimelineByDate", renderListStart);
  const renderListSource = appSource.slice(renderListStart, renderListEnd);
...
test("v036 mostra inteligência comercial antes do contexto financeiro", () => {
...
test("v036 aumenta a fonte das sugestões de resposta", () => {
...
test("v036 permite escolher o período dos áudios com 90 dias pré-selecionado", () => {
...
test("v036 transcreve apenas áudios novos e marca os antigos fora do período", () => {
...
test("v036 mostra progresso real, atividade contínua e tempo decorrido", () => {
...
test("v036 permite cancelar sem salvar atendimento parcial", () => {
...
test("v036 aumenta os textos da análise sem alterar as sugestões já aprovadas", () => {
...
test("v036 não repete aguardando resposta dentro da inteligência comercial", () => {
