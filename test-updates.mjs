...
test("v040 mantém atualização automática e evita mistura de arquivos em cache", () => {
  ...
  assert.match(versionSource, /app: "v064"/);
  ...
  assert.match(htmlSource, /app\.js\?v=064/);
  assert.match(htmlSource, /styles\.css\?v=064/);
  assert.match(appSource, /db\.js\?v=064/);
  assert.match(appSource, /whatsapp\.js\?v=064/);
...
test("versão v040 aparece no cabeçalho superior", () => {
  assert.match(htmlSource, /id="header-version"[^>]*>v064<\/span>/);
...
test("v041 usa uma fonte central de versão em app, servidor, build e cache", () => {
  ...
  assert.match(versionSource, /app: "v064"/);
  assert.match(versionSource, /package: "0\.64\.0"/);
...
test("v064 lê o print da conversa e mescla as mensagens na linha do tempo", () => {
...
test("v064 unifica identidade do corretor e corrige plural de mensagens", () => {
